...
The default tool protocol remains `prompt_json`. For llama.cpp server only, a profile can opt into native OpenAI-compatible tool calls with `"toolProtocol":"llamacpp_tools"`. Phase 1 uses non-stream chat completions, sends CodeRenga-generated built-in tool JSON Schemas, defaults `tool_choice` to `auto`, and forces `parallel_tool_calls:false` even if `parallelToolCalls:true` is configured. Start `llama-server` with `--jinja` and a tool-aware chat template; otherwise keep using `prompt_json`.

Compatibility note: `extraBody` is additive-only provider configuration. It cannot override CodeRenga-managed request fields such as `model`, `messages`, `stream`, `tools`, `tool_choice`, or `parallel_tool_calls`. Use `toolChoice` instead of `extraBody.tool_choice`; `parallelToolCalls` is reserved for future native-tools providers or a later llama.cpp tools phase. Native `tools` are generated from the tool registry and schemas, so `nativeTools` and `extraBody.tools` in JSON configuration cannot inject or replace them.
